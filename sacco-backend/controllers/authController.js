...
exports.register = async (req, res) => {
  ...
  try {
    const user = await User.create({ name, email, password: hash, phone, role, memberCode });
    res.json(user);
  } catch (err) {
...
exports.login = async (req, res) => {
  ...
  // Only allow admin login for the fixed admin account
  if (user.role === 'admin' && email !== 'admin@saccosmart.com') {
    return res.status(403).json({ message: 'Only the default admin can log in as admin' });
  }
